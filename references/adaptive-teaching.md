...
- **Diagnose first, then calibrate; recalibrate as the learner changes.**
  ...
  explanations the user has outgrown (the adaptive complement of the
...
4. **Calibrate per axis against the learner model; compress the known.**
   ...
   *impairs* high-prior-knowledge learners (expertise reversal: Kalyuga et
   ...
   O'Reilly & McNamara 2007). Calibrate per *axis*, not globally: the same
   user can be expert on one axis of a question and novice on another
   (research-level mathematics, unfamiliar Lean internals), and each axis's
   level is whatever `learner/model.md` says it is. Interesting
...
5. **Mechanism, not verdict — and demonstrate rather than assert.** A bare
   ...
   standing rule that demonstrations must be honest, and it is the
...
7. **Signal sparsely.** When the answer has multiple points, number them
   ...
   formatting — one signal per scope.
